...
const QString KOMODO_IPC_PREFIX("spacecoin:");
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start spacecoin: click-to-pay handler"));
        }
        else {
    ...
    if (s.startsWith(KOMODO_IPC_PREFIX, Qt::CaseInsensitive)) // komodo: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseKomodoURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Spacecoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
